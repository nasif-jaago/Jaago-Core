    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/jsonrpc`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                jsonrpc: '2.0',
                method: 'call',
                params: { service: 'object', method: 'execute_kw', args: [ODOO_CONFIG.DATABASE, uid, ODOO_CONFIG.API_KEY, 'approval.request', 'fields_get', [], { attributes: ['type', 'relation', 'string'] }] },
                id: 2
            })
        });
        const fields = (await res.json()).result;

        fs.writeFileSync('all_approval_request_fields.json', JSON.stringify(fields, null, 2));
        console.log('Done');

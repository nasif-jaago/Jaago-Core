...
import RequisitionDynamicFieldRenderer from './RequisitionDynamicFieldRenderer';

interface RequisitionCreatePageProps {
    categories: RequisitionCategory[];
    fields: Record<string, RequisitionField>;
    onBack: () => void;
    onSuccess: () => void;
}

const RequisitionCreatePage: React.FC<RequisitionCreatePageProps> = ({ categories, fields, onBack, onSuccess }) => {
    const [formData, setFormData] = useState<RequisitionFormValues>({
        name: '',
    ...
    const [selectedCategory, setSelectedCategory] = useState<RequisitionCategory | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await createRequisitionRequest(submitData);

            if (result.success) {
                onSuccess();
            } else {
                setErrors({ submit: result.error || 'Failed to create requisition request' });
            }
        } catch (err: any) {
        ...
        <div className="fade-in" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '2px' }}>Requisition</p>
                        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-main)' }}>New requisition request</h2>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', flex: 1, minHeight: 0 }}>
                            ...
                            {activeTab === 'standard' ? (
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                                    {studioFields.map(field => (
                                        <div key={field.name}>
                                            <RequisitionDynamicFieldRenderer
                                                field={field}
                                    ...
                                    {studioFields.length === 0 && (
                                        <div style={{ gridColumn: '1 / -1', padding: '4rem', textAlign: 'center' }}>
                                            ...
                                            <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem' }}>No custom Odoo Studio fields are currently active for Requisition.</p>
                                        </div>
                                    )}
                                </div>
                            )}
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div className="card" style={{ padding: '1.5rem', background: 'var(--primary-glow)' }}>
                        <h5 style={{ fontWeight: 700, marginBottom: '0.75rem', color: 'var(--primary)' }}>Requisition Process</h5>
...
export default RequisitionCreatePage;

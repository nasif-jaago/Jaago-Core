...
const RequisitionDetailModal: React.FC<RequisitionDetailModalProps> = ({ requisition, onClose, onUpdate }) => {
    ...
    const handleApprove = async () => {
        ...
        setError(null);
        try {
            const result = await approveRequest(requisition.id);
            if (result.success) {
                onUpdate();
                onClose();
            } else {
    ...
    const handleRefuse = async () => {
        ...
        setError(null);
        try {
            const result = await refuseRequest(requisition.id);
            if (result.success) {
                onUpdate();
                onClose();
            } else {
    ...
    const handleCancel = async () => {
        ...
        setError(null);
        try {
            const result = await cancelRequest(requisition.id);
            if (result.success) {
                onUpdate();
                onClose();
            } else {
        ...
        >
            ...
            >
                ...
                <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <span style={{
                            ...
                            fontSize: '0.75rem',
                            fontWeight: 700,
                            color: getStatusColor(requisition.request_status),
                            background: `${getStatusColor(requisition.request_status)}20`,
                            border: `1px solid ${getStatusColor(requisition.request_status)}40`
                        }}>
                            {requisition.request_status.toUpperCase()}
                        </span>
                    </div>
                    <button
                        onClick={onClose}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                            <div>
                                ...
                                <p style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-main)' }}>
                                    {requisition.request_owner_id ? requisition.request_owner_id[1] : 'N/A'}
                                </p>
                            </div>
                            <div>
                                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '4px' }}>Category</p>
                                <p style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-main)' }}>
                                    {requisition.category_id ? requisition.category_id[1] : 'N/A'}
                                </p>
                            </div>
                            <div>
                                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '4px' }}>Request Date</p>
                                <p style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-main)' }}>
                                    {requisition.date ? new Date(requisition.date).toLocaleDateString() : 'N/A'}
                                </p>
                            </div>
                            {requisition.amount && (
                                <div>
                ...
                <div style={{
                    ...
                    {requisition.request_status === 'new' || requisition.request_status === 'pending' ? (
                        <>
                            <button

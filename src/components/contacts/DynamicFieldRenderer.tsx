...
import { fetchRelationOptions } from '../../api/ContactsService';
...
const DynamicFieldRenderer: React.FC<DynamicFieldRendererProps> = ({ field, value, onChange, readonly = false }) => {
    ...
    const loadOptions = async () => {
        ...
        // Basic fetch with limited records. For real production, this needs search/autocomplete
        const res = await fetchRelationOptions(field.relation, [], 100);
            ...
            return (
                <div style={{ marginBottom: '1rem' }}>
                    ...
                    <input
                        ...
                        disabled={readonly}
                        step={field.type === 'float' || field.type === 'monetary' ? '0.01' : '1'}
                        style={{ width: '100%' }}
                    />
                </div>
            );

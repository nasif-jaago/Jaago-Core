    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/jsonrpc`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                jsonrpc: '2.0',
                method: 'call',
                params: { service: 'object', method: 'execute_kw', args: [ODOO_CONFIG.DATABASE, uid, ODOO_CONFIG.API_KEY, 'approval.category', 'read', [[25]], { fields: ['name', 'company_id'] }] },
                id: 2
            })
        });
        const cat = (await res.json()).result;

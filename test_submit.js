...
async function trySubmit() {
    console.log('Finding a draft request...');
    const draft = await odooCall('approval.request', 'search_read', [[['request_status', '=', 'new']]], { limit: 1 });
    if (draft.result && draft.result.length > 0) {
        const id = draft.result[0].id;
        console.log('Found draft ID:', id);
        console.log('Attempting action_confirm...');
        const res = await odooCall('approval.request', 'action_confirm', [[id]]);
        console.log('Result:', JSON.stringify(res, null, 2));
    } else {
        console.log('No draft found to test.');
    }
}
trySubmit();

    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/jsonrpc`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                jsonrpc: '2.0',
                method: 'call',
                params: { service: 'object', method: 'execute_kw', args: [ODOO_CONFIG.DATABASE, uid, ODOO_CONFIG.API_KEY, 'approval.request', 'fields_get', [], { attributes: ['string', 'type', 'relation'] }] },
                id: 2
            })
        });
        const fields = (await res.json()).result;

        const historyFields = Object.keys(fields).filter(k => k.toLowerCase().includes('history') || k.toLowerCase().includes('approver'));
        const results = {};
        for (const k of historyFields) {
            results[k] = fields[k];
        }

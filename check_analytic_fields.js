    ...
    const callRes = await fetch(ODOO_CONFIG.DOMAIN + '/jsonrpc', {
        ...
        body: JSON.stringify({
            ...
            params: {
                service: 'object',
                method: 'execute_kw',
                args: [ODOO_CONFIG.DATABASE, uid, ODOO_CONFIG.API_KEY, 'account.analytic.account', 'fields_get', [[]], { attributes: ['string'] }]
            },
            id: 2
        })
    });
    const fields = await callRes.json();
    console.log('--- Custom Analytic Fields ---');
    Object.entries(fields.result).filter(([k, v]) => k.startsWith('x_')).forEach(([k, v]) => {

...
async function main() {
    try {
        ...
        console.log('Fetching all field names for hr.appraisal...');
        ...
        const names = Object.keys(fields).filter(f =>
            f.includes('note') || f.includes('feedback') || f.includes('text') || f.includes('comment') || f.includes('response') || f.includes('x_studio')
        );
        const filteredFields = {};
        names.forEach(n => filteredFields[n] = fields[n]);
        console.log('Potential Content Fields:', JSON.stringify(filteredFields, null, 2));

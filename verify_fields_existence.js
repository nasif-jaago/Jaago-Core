...
async function main() {
    try {
        ...
        console.log('Checking for next_appraisal_date on hr.employee and hr.appraisal...');
        const empFields = await call('object', 'execute_kw', [
            uid, ODOO_CONFIG.API_KEY,
            'hr.employee', 'fields_get',
            [['next_appraisal_date']],
            { attributes: ['string'] }
        ]);
        console.log('Employee next_appraisal_date:', empFields.next_appraisal_date ? 'Exists' : 'MISSING');

        const appFields = await call('object', 'execute_kw', [
            uid, ODOO_CONFIG.API_KEY,
            'hr.appraisal', 'fields_get',
            [['next_appraisal_date']],
            { attributes: ['string'] }
        ]);
        console.log('Appraisal next_appraisal_date:', appFields.next_appraisal_date ? 'Exists' : 'MISSING');

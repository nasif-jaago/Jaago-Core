    ...
    try {
        ...
        const res = await fetch(`${BASE_URL}/jsonrpc`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
                jsonrpc: '2.0',
                method: 'call',
                params: { service: 'object', method: 'execute_kw', args: [ODOO_CONFIG.DATABASE, uid, ODOO_CONFIG.API_KEY, 'approval.request', 'fields_get', [], { attributes: ['type', 'relation'] }] },
                id: 2
            })
        });
        ...
        for (const [k, v] of Object.entries(fields)) {
            if (v.type === 'one2many' || v.type === 'many2many') {
                results[k] = v;
            }
        }
